<template>
  <div>
    ...
    <v-container v-if="!userPosts.length && !loading">
      <v-row>
    ...
    <v-container v-else>
      <v-row>
        <v-col cols="12" sm="6" md="4" v-for="post in userPosts" :key="post._id">
          <v-card flat class="mt-3" hover>
            <v-card flat>
              <v-img height="30vh" v-ripple @click="goToPost(post._id)" :src="post.imageUrl"></v-img>
              <v-btn
                color="info"
                fab
                left
